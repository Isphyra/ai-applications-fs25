...
def predict_apartment(rooms, area, town):
    bfs_number = supermarket_locations[town]
...
iface = gr.Interface(
    ...
    inputs=[
         gr.Number(label="Anzahl der Zimmer"),
         gr.Number(label="Fläche in m²"),
         gr.Dropdown(choices=list(supermarket_locations.keys()), label="Stadt", type="value")
    ],
    outputs=[gr.Number(label="Geschätzter Apartment Preis")],
    title="Apartment Price Berechnung in der Schweiz",
    description="Dieses Modell berechnet den Preis einer Wohnung in der Schweiz basierend auf der Anzahl der Zimmer, der Fläche, die in der Nähe eines Supermarktes liegt."
)
